...
const closeDetailModal = () => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await fetch('http://localhost:5000/users'); // Adjust the API endpoint as needed
      if (!response.ok) throw new Error('Network response was not ok');
      const data = await response.json();
      setUsers(data); 
    } catch (error) {
  ...
  const handleUsernameChange = (event) => {
    ...
    setFormData({
      ...
      nama_perusahaan: selectedUser ? selectedUser.nama_perusahaan : '',
    });
  };
  ...
  return (
    <div className="perusahaan">
      ...
      <AddPerusahaan
        ...
        onUsernameChange={handleUsernameChange} // Pass the username change handler
        onSubmit={handleSubmit}
        users={users} // Pass the list of users to the AddPerusahaan component
      />
      <ConfirmDeleteModal
     ...
     <DetailPerusahaanModal
      ...
      perusahaan={selectedPerusahaan} // Pastikan nama props sesuai
    />
